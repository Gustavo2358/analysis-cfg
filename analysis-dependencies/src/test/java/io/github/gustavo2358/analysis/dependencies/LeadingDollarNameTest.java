...
final class LeadingDollarNameTest {
    ...
    @Test void syntheticLeadingDollarWithPaddingIsSupported() {accepted("$TEST123   ",true,"$TEST123");}
    ...
    @Test void existingAcceptedFormsKeepTheirExactNames() {
        for(var name:new String[]{"PROGA","A$PROG","_PROGA","ABCDEFGH"}) {
            accepted(name,false,name);accepted(name+"   ",true,name);
        }
    }
    @Test void existingRejectionsAndLiteralPaddingRemainOpen() {
        for(var raw:new String[]{" PROGA"," $PROGA","proga","$proga","PROG-A","$PROG-A","ABCDEFGHI","$ABCDEFGH","","   ","$PROGA\t","$PROGA\u00a0","$PRO GA","@PROGA","#PROGA"}) {
