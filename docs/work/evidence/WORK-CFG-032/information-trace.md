...
| Caller source identity/display | cobol-lower → AIR | SpInput.UnitKey.canonicalProgramName; LoweringResult correlations; UnitId(pub,"unit") | Identidade técnica sim; nome não é campo Unit AIR | Correlations/origins presentes em memória | N/A | N/A | PARTIAL (L11–L14) | CLI exporta Publication, não EntryLink/StatementLink; nome CALLER indisponível pelo AIR JSON isolado | UnitId obrigatório no fact; display opcional via correlação explícita/receipt, sem parsing nem catálogo |
