# BACKLOG-CFG-014 — Semântica local: frames e portas de conclusão

**Estado:** `planned`. **Fase:** `local`. **Autorização:** backlog não autoriza execução.
Dependências: BACKLOG-CFG-011, BACKLOG-CFG-013.
...
TDD local.invoke, boundary, resume/unwind, identities, default, erros e representação aprovada; contextos de teste pequenos independentes.

## Critérios de aceitação

Sem execução direta invoke→resume; topo-only; duas boundaries podem usar mesma porta; jump não desempilha; não criar nova ativação; corpo mantém operação compartilhada.

## Evals e invariantes

EVAL-CFG-016, EVAL-CFG-017, EVAL-CFG-018, EVAL-CFG-009. Vincular invariantes específicos na promoção para work
...
Sem classificar PERFORM por nome, clonar corpo por callsite por padrão ou alegar precisão de queries ainda não implementadas.
