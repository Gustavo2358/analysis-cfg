...
## MVP-CFG-01: fluxo linear e bifurcação

Domínio positivo: uma publicação válida, units/entries explicitadas; sequences com
operações comuns de fluxo linear; `jump`, `branch` e saídas `return`/`halt`. Uma
fixture de integração pode conter `invoke` com outcomes materializados (normal,
excepcional, halt/diverge quando pertinentes). Predicado puro desconhecido conserva
os dois destinos. IFs aninhados são combinações das mesmas primitives, sem limite
artificial de um IF ou um CALL. Não inventar estado de dados para desenhar o grafo.
...
O resultado não resolve o programa chamado nem afirma ausência de efeitos.
