...
`SELECT CUSTOMER-FILE ASSIGN TO CUSTOMERDD` pode terminar em
`programa → logical file CUSTOMER-FILE → DDNAME CUSTOMERDD`.
Logical file e DDNAME coexistem porque ambos vêm do COBOL. DDNAME é identidade
terminal válida, assim como `CALL 'XPTO123'` termina em programa → XPTO123.
...
| Alvo source-level | DDNAME/assignment-name, literal de dialeto ou nome CICS FILE |
...
| PARTIAL | dimensão semântica incompleta: inventário, binding, valores, efeitos ou controle |
...
5. SD não ganha DDNAME persistente inventado. WRITE/REWRITE usam record ownership;
...
7. Não confundir namespaces CICS FILE, DDNAME e conector por igualdade de grafia.
...
transformada em execução, ou promoção de UNKNOWN obrigatório a implementação
