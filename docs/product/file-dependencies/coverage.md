...
| C06 Aliases e configurações | QUALIFIED — READ DATASET por [C06 humano](c06-read-dataset.md), SET aliases por C-FC | READ DATASET e SET DATASET/OBJECTNAME individualmente autorizados; outros aliases não são generalizados | T40,T41 | W8 |
...
## Qualificação final W11 — QUALIFIED_LOCAL

N+C selecionado qualificado; [W11](w11-qualification.md) separa gates novos e
REUSED. As101 fontes×2, MR/SG, escala e mutantes anteriores são preservados;
a retomada C06 acrescenta41CICS×2, pares FILE/DATASET literal/computed e21
programas do corpus.35 novos sites FILE, três deles em COACTVWC, com spelling,
binding e provenance provados.41 vetores CALL inalterados.

C06: READ DATASET→FILE autorizado por C06-HUMAN-20260917, com contrato idêntico
READ FILE. SET DATASET→FILE e OBJECTNAME→atributo administrativo preservados.
Nenhum namespace/target DSNAME ou lookup físico. As52 ocorrências observadas de
DATASET em outros comandos permanecem NOT_QUALIFIED, registradas separadamente
conforme limite humano; não são promovidas por analogia nem reclassificadas D.
W8/W11 QUALIFIED_LOCAL nesse escopo explícito. W10 TODO/NOT_AUTHORIZED. STOP.
