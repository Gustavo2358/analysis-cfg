...
N-LR governa N01–N17/N19–N33. N04 aplica-se ao perfil D; N18 é D Report Writer.
...
| N02 `ASSIGN TO assignment-name` IBM | PLANNED / N-LR | `assignClause` → Nome escrito; componentes interpretados segundo perfil; alvo source-level (tipicamente DDNAME); Não tratar automaticamente um nome sem aspas como variável COBOL. | T02,T30 | W0 |
| N03 Prefixos/comentários da assignment-name | PLANNED / N-LR | `assignClause` → Nome original e nome interpretado, regra/versionamento; Não aplicar corte universal por hífen. | T30 | W0/W6 |
| N04 ASSIGN literal/dinâmico/externo em dialetos | PLANNED / D-GC32 | `assignClause` → Variante tipada e expressão/alvo de associação no próprio fonte; Dialeto selecionado; literal, data item e ambiente não colapsam. | T31–T35 | W7 |
...
| N08 FD versus SD | PLANNED / N-LR | `fileDescriptionEntry` → Espécie explícita e associação a SELECT; SD/sort-work não produz DDNAME ou alvo persistente inventado quando o fonte não o fornece. | T01,T23 | W0/W5 |
...
| N18 REPORT(S)/RD em dialetos com Report Writer | PLANNED / D-GC32 | `reportClause/reportSection` → Ligação relatório → arquivo; registros/controle pertinentes; Relatório não vira programa chamado. | D-RW | W10 |
...
| N24 `DELETE ... RECORD` | PLANNED / N-LR | `deleteStatement` → File-name; Exclusão de registro, não criação de uma nova dependência nem inferência de remoção externa. | T09 | W2 |
...
## D — subset finito, autoridade D-GC32/D-v1

Cada API D-v1 da página de perfis exige uma sublinha no catálogo da W10 com
assinatura/operandos/efeitos/outcomes/oráculo. D07 tem somente teste negativo.

| ID / família | Estado | Contrato / efeito | Oráculo | Wave |
| --- | --- | --- | --- | --- |
| D01 ASSIGN dinâmico/ambiente e remapeamento | PLANNED | Política versionada, contexto explícito; nenhum uso do ambiente real do analisador. | T31–T35 | W7/10 |
...
Semântica obrigatória ainda UNSUPPORTED impede encerrar seu perfil em W11.
