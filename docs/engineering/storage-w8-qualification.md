...
Regional effects/RD/values reuse the interval domain, preserve captures and simultaneous supports, observe CALL before foreign effects, and publish detached regional evidence. Sparse padding at extent 2^100 remains one span; literal/output payload costs are still proportional to actual data.

## Validation

Full Java suite: 458 tests passed; 2 scale tests/9 shapes passed separately. All current architecture and wire boundaries, 38 project Python tests and 14 lean policy tests passed. W1–W5 performance metric validators passed.
