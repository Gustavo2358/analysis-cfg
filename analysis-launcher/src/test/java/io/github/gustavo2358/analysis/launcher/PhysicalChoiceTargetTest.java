...
class PhysicalChoiceTargetTest {
 ...
 @Test void physicalAlternativesSurviveMemoryAndCliWithoutResurrectingMustValues() throws Exception {
  ...
  for(var name:List.of("ambiguous","ambiguous-must","ambiguous-mixed")) {
   ...
   var p=new AirJson().decode(bytes);var result=new DependencyAnalysis().prepare(p);var site=result.sites().getFirst();
   ...
   var errors=new ByteArrayOutputStream();assertEquals(0,AnalysisDependencies.run(new String[]{path.toString(),output.toString()},new PrintStream(errors)),errors.toString());
   ...
   if(name.equals("ambiguous")) {
    ...
    var regional=new io.github.gustavo2358.analysis.dataflow.RegionalAnalysis().prepare(p,"choice-query",List.of(query));
