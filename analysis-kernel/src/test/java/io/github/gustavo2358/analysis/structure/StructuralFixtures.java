...
final class StructuralFixtures {
    private StructuralFixtures() { }
    ...
    static Operations.Assign assign(UnitId u, String id, ObjectId object) {
        var h=header(u,id);
        return new Operations.Assign(h,new Places.ObjectPlace(operand(h.id(),"destination",Operand.Role.VALUE_WRITE),object),
                new Expressions.Literal(operand(h.id(),"value",Operand.Role.VALUE_READ),new Values.TextValue("literal")));
    }
