...
def validate_audit(root: Path) -> list[str]:
    ...
    try:
        ...
        for key in OBLIGATIONS:
            expected_hooks = {str(n):{'status':'IMPLEMENTED','hook':f'scripts/project/check_w{n}.py'} for n in (1,2) if n in WAVES[key]}
            require(data['requirements'][key].get('wave_hooks',{}) == expected_hooks, 'W1/W2 audit hook '+key)
