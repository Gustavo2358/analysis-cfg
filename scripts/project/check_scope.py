#!/usr/bin/env python3
"""Check CP5 Wave 2 diff scope, fixed upstream authority and a complete tracked/untracked delivery manifest."""
...
def manifest() -> bytes:
    lines = ["# SHA-256 da entrega CP5 W2; inclui arquivos versionáveis, exclui este manifesto e artefatos ignorados."]
...
def main() -> int:
    ...
    try:
        ...
        approved = "b84389b6ccf94c259774b82a99bc7296278b65c0"
        if git("diff", "--name-only", approved, "--", "cfg-kernel", "cfg-adapters", "cfg-launcher/src", "docs/sources/sources.lock.json"):
            raise ValueError("unapproved product/pin changes outside W2")
        ...
        if work["checkpoint"] != "WAVE_2":
            raise ValueError("scope checker authorizes WAVE_2 only")
        ...
        # The aggregate PR includes historic AGENTS edits; W2 refreshes the architecture overview.
        ...
        focal_scope = ["ARCHITECTURE.md", "docs", "scripts/harness", "scripts/project", ".github/workflows/ci.yml", "MANIFEST.sha256", "pom.xml", "cfg-launcher/pom.xml", "analysis-kernel"]
        ...
        if before != after: raise ValueError("W2 scope: approved audit semantics changed")
        for path in ["scripts/project/ci_source_receipt.py", "docs/evals/cp5/result-contract.json", "docs/evals/cp5/result-review.json", "docs/evals/cp5/phase-review.json", "docs/evals/cp5/core-size-review.json"]:
            if (ROOT/path).read_bytes() != git("show", remediation_base + ":" + path):
                raise ValueError("W2 scope: future-wave contract/CI receipt changed: " + path)
        ...
        if any(not p.startswith("docs/work/evidence/WORK-CFG-028/wave-2/") for p in historical):
        ...
        git("diff", "--check")
        print("[scope/manifest] PASS: CP4 + approved W1 baseline, unchanged legacy Java/pins, W2 production scope/DRAFT, AIR fixture hash and exact delivery manifest")
        return 0
