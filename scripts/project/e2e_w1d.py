...
def run(work,config):
    ...
    cp=os.pathsep.join([str(ROOT/m/'target/classes') for m in modules]+[runtime]);commands=[];cases={}
    ...
    receipt=dict(schema='cp6-w1d-e2e@1',producers=str(producer/'producers.json'),cases=cases,commands=commands,injections=False,determinism='BYTE_IDENTICAL')
