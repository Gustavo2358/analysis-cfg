...
def freeze(root,work,m2,allow_dirty=False):
    ...
    config=dict(sources=sources,checkouts={r:str(p) for r,p in checkouts.items()},semanticProductVersion='2.6.0',jarSha256=hashes,
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--root',type=Path,required=True);p.add_argument('--work',type=Path,required=True);p.add_argument('--m2',type=Path,required=True);p.add_argument('--allow-dirty',action='store_true')
    a=p.parse_args();freeze(a.root.resolve(),a.work.resolve(),a.m2.resolve(),a.allow_dirty)
