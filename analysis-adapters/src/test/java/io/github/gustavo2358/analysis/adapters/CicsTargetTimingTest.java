...
class CicsTargetTimingTest {
    @Test void nameAndSupportsAreObservedBeforeForeignEffects() throws Exception {
        ...
        var values=RegionalValuesAnalysis.prepare(session).analysis().orElseThrow().execute().observeStorage(List.of(before,after)).observations();
        ...
        var fact=new DependencyAnalysis().prepare(p).sites().getFirst();assertEquals(before.point(),fact.valuePoint());assertFalse(fact.modelValueRemainder());
