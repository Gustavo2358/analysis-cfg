...
final class FileCheckpointOracleTest {
    @Test void optionalLoopPreservesExactValueUnlessWritesAreActuallyAllowed()throws Exception {
        for(boolean writes:List.of(false,true)) {
            ...
            assertEquals(List.of("SAFE0001"),call.candidates().stream().map(DependencySiteFact.Candidate::referenceName).toList());assertEquals(writes,call.modelValueRemainder(),"read-only visible scope cannot open the value");assertEquals(0L,result.metrics().get("physicalGroupsApplied"));
