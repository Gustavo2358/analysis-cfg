...
class RegionalScaleQualificationTest {
    static void measure(String shape,int n,Publication p,int expectedCandidates,int expectedSupports) {
        long start=System.nanoTime();var session=session(p);long indexed=System.nanoTime();var admission=RegionalValuesAnalysis.prepare(session);assertEquals(RegionalValuesAnalysis.Status.ACCEPTED,admission.status());long prepared=System.nanoTime();
